...
DROP PROCEDURE IF EXISTS GetPostSearchDetail2;

delimiter //

create procedure GetPostSearchDetail2(_offset int, _rowcount int)
begin
...
DROP PROCEDURE IF EXISTS eraseThread;
delimiter //
CREATE PROCEDURE eraseThread (in _threadId bigint)
begin

delete post_edit from post_edit inner join post on post_edit.post_id = post.id where post.discussion_id = _threadId;
delete moderator_comment from moderator_comment inner join post on moderator_comment.post_id = post.id where post.discussion_id = _threadId;
delete post_report from post_report inner join post on post_report.post_id = post.id where post.discussion_id = _threadId;
delete from post where discussion_id = _threadId;

delete subscription from subscription inner join discussion on subscription.discussion_id = discussion.id where discussion.id = _threadId;
delete folder_subscription_exception from folder_subscription_exception inner join discussion on folder_subscription_exception.discussion_id = discussion.id where discussion.id = _threadId;
delete user_discussion from user_discussion inner join discussion on user_discussion.discussion_id = discussion.id where discussion.id = _threadId;
delete from discussion where id = _threadId;

delete from front_page_entry where discussion_id = _threadId;
