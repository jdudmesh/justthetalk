...
class PostService {
    ...
    def savePost(discussionId, postId, postText, folderId, sourceId, sourceType, postAsAdmin, ip) {
		...
		if(postText == null || postText.trim().length() == 0) {
			throw new InvalidPostException(reason: "You must enter some text")
		}
		...
		if(discussionUser && discussionUser.userStatus == DiscussionUser.STATUS_BLOCKED) {
			throw new InvalidPostException(reason: "You may not post on this thread")
		}
	...
	*/
	...
	def countPostsBetween(startDate, endDate) {
		def numPosts = Post.executeQuery("select count(*) from Post r where r.createdDate >= :startDate and r.createdDate < :endDate", [startDate: startDate.toDate(), endDate: endDate.toDate()])[0]
		return numPosts
	}

	def countUniqueUsersBetween(startDate, endDate) {
		def numPosts = Post.executeQuery("select count(distinct user) from Post r where r.createdDate >= :startDate and r.createdDate < :endDate", [startDate: startDate.toDate(), endDate: endDate.toDate()])[0]
		return numPosts
	}

	def countReportsBetween(startDate, endDate) {
		def numReports = PostReport.executeQuery("select count(*) from PostReport r where r.createdDate >= :startDate and r.createdDate < :endDate", [startDate: startDate.toDate(), endDate: endDate.toDate()])[0]
		return numReports
	}

	def countReportResultsBetween(startDate, endDate) {
		def numReports = PostReport.executeQuery("select p.moderationResult, count(*) from Post p inner join p.reports r where r.createdDate >= :startDate and r.createdDate < :endDate group by  p.moderationResult", [startDate: startDate.toDate(), endDate: endDate.toDate()])
		return numReports
	}
