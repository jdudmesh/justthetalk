...
class AdminController {
	...
	def moderate = {
		...
		nav.start = 0
		render view: "moderate", model:[posts:posts, user:springSecurityService.currentUser, nav: nav, pageTitle:"Moderate"]
	}
	...
	def modhistory = {
		...
		nav.start = 0
		render view: "moderate", model:[posts:posts, user:springSecurityService.currentUser, nav: nav, pageTitle:"Moderation History"]
	...
	def resetpassword = {
		...
		redirect2 controller: "home", action: "user", id: user.username
	}
	...
	def moderateuser = {
		...
		println this.getClass().getSuperclass().getName()
		redirect2 controller: "home", action: "user", id: user.username
	...
	def reports() {
		if (request.method == 'GET') {
			render view:"reports", model: [startDate:null, endDate:null]
		}
		else {
			println params.toString()
			def startDate = formatter.parseDateTime(params.startDate)
			def endDate = formatter.parseDateTime(params.endDate).plusDays(1)
			def numPostsBetween = postService.countPostsBetween(startDate, endDate)
			def numUsersBetween = postService.countUniqueUsersBetween(startDate, endDate)
			def numReportsBetween = postService.countReportsBetween(startDate, endDate)
			def numResults = postService.countReportResultsBetween(startDate, endDate)
			render view:"reports", model: [startDate:params.startDate, endDate:params.endDate, numPostsBetween:numPostsBetween, numUsersBetween: numUsersBetween, numReportsBetween:numReportsBetween, numResults:numResults]
		}
	}
