    ...
    def init = { servletContext ->
		...
		*/
		...
		exceptionHandler.exceptionMappings = [
			'org.codehaus.groovy.grails.web.servlet.mvc.exceptions.ControllerExecutionException' :'/invalidarg']
		*/
		...
		org.codehaus.groovy.grails.web.sitemesh.GrailsContentBufferingResponse.metaClass.sendProtoAwareRedirect = { request, location ->

			def proto = request.getHeader("X-Forwarded-Proto")
			def host = request.getHeader("Host")
			if(proto && proto.toUpperCase() == "HTTPS") {
				location = "https://${host}${request.getContextPath()}${location}"
			}
			else {
				location = "http://${host}${request.getContextPath()}${location}"
			}
		...
		grailsApplication.controllerClasses.each { c ->
			...
			c.metaClass.redirect2 = { Object params ->

				def location = "/"

				if(java.util.LinkedHashMap.class.isInstance(params)) {
					location = createLink(params)
				}
				else {
					location = "${request.getContextPath()}${params}"
				}
