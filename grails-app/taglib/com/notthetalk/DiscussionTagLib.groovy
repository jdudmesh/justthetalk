...
class DiscussionTagLib {
	...
	def discussionTitle = { attrs, body ->
		def showUnsub = false
		if(attrs.showUnsub) {
			showUnsub = attrs.showUnsub
		}

		out << renderDiscussionTitle(attrs.header, attrs.max, attrs.showfolders, showUnsub)
	}
	...
	def renderDiscussionTitle(dis, max, showFolders, showUnsub) {
		...
		b.li(class:"discussionlistitem") {

			if(showUnsub) {
				input(type:"checkbox", name:"unsub", value:dis.discussionId, style:"margin-right:4px")
			}

			a(href: g.createLink(uri:dis.canonicalUrl([do:"check"]))) {
				mkp.yieldUnescaped(TextFormatter.renderLineBreaks(dis.discussionName, Integer.parseInt((String)max)))
			}

			if((boolean)showFolders) {
				mkp.yield(" - ")
				a(href: g.createLink(uri:"/$dis.folderKey")) {
